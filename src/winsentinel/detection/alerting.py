...
from winsentinel.core.models import DetectionResult, NetworkContext
from winsentinel.core.models.alert import TERMINAL_STATUSES, Alert, AlertStatus
from winsentinel.detection.scoring import RiskScore, score_detections
...
def recommended_actions(risk: RiskScore, running: bool) -> tuple[str, ...]:
    ...
    actions = ["Inspect the process with 'winsentinel inspect <PID>'"]
    if running:
        actions.append(
            "Suspend it to freeze activity while you investigate: 'winsentinel suspend <PID>'"
        )
        if risk.score >= 60:
            actions.append(
                "Terminate it if you confirm it is unwanted: 'winsentinel terminate <PID>'"
            )
    actions.append("If it is expected, allowlist it by path or SHA256 (see 'winsentinel allow')")
