...
counted (surfaced in ``winsentinel status``) rather than growing memory without bound.
"""
...
from winsentinel.storage.repositories import SecurityStore
...
class DatabaseWriter:
    ...
    def start(self) -> None:
        self._thread = threading.Thread(target=self._run, name="winsentinel-db-writer", daemon=True)
        self._thread.start()
