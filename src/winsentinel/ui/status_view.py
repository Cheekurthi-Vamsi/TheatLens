"""Rendering for ``winsentinel status``."""
...
from winsentinel.core.diagnostics import CheckResult
from winsentinel.core.models import ComponentStatus, EngineStatus
from winsentinel.ui import colors
from winsentinel.ui.formatting import format_bytes, format_percent, sanitize_display
...
def status_view(
    *,
    ...
    if engine is not None and engine_live:
        ...
        overview.add_row(
            "Engine", Text("NOT RUNNING  (start it with 'winsentinel monitor')", style=colors.MUTED)
        )
    ...
    return Panel(Group(*parts), title="WINSENTINEL STATUS", title_align="left", border_style="cyan")
