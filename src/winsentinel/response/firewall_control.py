...
interception. ``netsh advfirewall firewall`` is that interface. WinSentinel:

* only ever *adds* block rules and *removes* rules it created (every rule name is prefixed
  ``WinSentinel:``), so it can never weaken the firewall or touch the user's own rules;
...
from winsentinel.utils.networking import validate_ip, validate_port

RULE_PREFIX: Final = "WinSentinel:"
_NETSH: Final = "netsh"
_TIMEOUT: Final = 15.0


class FirewallError(WinSentinelError):
...
class FirewallController:
    ...
    def remove(self, rule_name: str) -> None:
        if not rule_name.startswith(RULE_PREFIX):
            raise FirewallError(
                f"refusing to delete a rule not created by WinSentinel: {rule_name!r}"
            )
    ...
    def list_rules(self) -> list[str]:
        """WinSentinel-created rule names currently present."""
        result = self._backend.run(
