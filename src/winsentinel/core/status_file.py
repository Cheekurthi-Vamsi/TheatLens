...
A running ``winsentinel monitor`` periodically writes :class:`EngineStatus` to
``%LOCALAPPDATA%\\WinSentinel\\engine-status.json``; ``winsentinel status`` (a separate process)
...
from winsentinel.config import app_data_dir
from winsentinel.core.models import EngineState, EngineStatus
...
class EngineAlreadyRunningError(WinSentinelError):
    ...
    def __enter__(self) -> Self:
        ...
        try:
            ...
            raise EngineAlreadyRunningError(
                "Another WinSentinel monitor is already running for this user "
                f"(lock held on {self.path}). Use 'winsentinel status' to see it."
            ) from None
        self._fd = fd
        return self
