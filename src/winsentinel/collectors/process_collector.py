...
What Windows APIs are used?
    * ``NtQuerySystemInformation(SystemProcessInformation)`` — **one call per snapshot** returns
      ...
      per-thread wait state for every process (see :mod:`winsentinel.utils.ntapi`).
    * For each *newly seen* process, one ``OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION)``
      ...
      See :mod:`winsentinel.utils.windows`.
...
from winsentinel.security.hashing import FileHasher, FileTooLargeError, NotARegularFileError
...
from winsentinel.utils.ntapi import (
    HUNDRED_NS_PER_SECOND,
    SystemProcessEntry,
    SystemProcessQuery,
    query_image_name_by_pid,
)
from winsentinel.utils.time import from_epoch, utc_now
