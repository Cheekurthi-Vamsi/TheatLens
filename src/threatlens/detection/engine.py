...
from threatlens.core.interfaces import DetectionRule, StateView
...
from threatlens.correlation.process_tree import ancestry
from threatlens.detection.allowlist import AllowlistMatcher
...
class DetectionEngine:
    ...
    def exclude_process_keys(self, process_keys: Collection[str]) -> None:
        """Never report these exact process *instances* (used for ThreatLens's own processes).

        Exclusion is by process key (PID + creation time), not by name or path: a copy of
        threatlens.exe elsewhere, or a later instance with a reused PID, is still evaluated.
        """
        with self._lock:
...
LAUNCHER_NAMES: Final = frozenset({"python.exe", "pythonw.exe", "py.exe", "threatlens.exe"})


def own_process_keys(processes: Sequence[ProcessInfo], pid: int) -> set[str]:
    """Process keys of this ThreatLens instance and its launcher chain.

    ``threatlens.exe`` (pip's console-script stub) starts the venv's
