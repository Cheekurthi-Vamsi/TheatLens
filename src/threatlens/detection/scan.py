"""One-shot evaluation of a single process, used by ``threatlens inspect``.
...
What this can and cannot find:
    ...
    * Rules that need to *watch change over time* (a listener opening, first-seen communication,
      connection bursts) cannot fire from a single snapshot; run ``threatlens monitor`` for those.
"""
