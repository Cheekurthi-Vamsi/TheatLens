...
from threatlens.core.diagnostics import process_instance_alive, run_self_test
...
from threatlens.detection.allowlist import AllowlistMatcher, AllowlistMatchType, entry_from_row
...
from threatlens.detection.rules import build_rules, rule_catalog
...
from threatlens.errors import ExitCode, InvalidInputError, ProcessNotFoundError, ThreatLensError
from threatlens.logging_config import configure_logging
...
from threatlens.response.response_manager import ResponseManager, current_user
from threatlens.security.hashing import FileHasher
from threatlens.security.privileges import ELEVATION_HINT, PrivilegeState, detect_privileges
from threatlens.security.signatures import SignatureVerifier
from threatlens.storage.database import Database, loads
from threatlens.storage.repositories import DatabaseInfo, SecurityStore, alert_from_row
from threatlens.storage.writer import DatabaseWriter
from threatlens.ui import colors
from threatlens.ui.alert_views import alert_detail, alerts_table, breakdown_text
from threatlens.ui.baseline_views import (
    allowlist_table,
    baseline_compare_view,
    baselines_table,
)
from threatlens.ui.dashboard import Dashboard
from threatlens.ui.detection_views import detections_section, rule_detail, rules_table
from threatlens.ui.event_stream import ALL_CATEGORIES, DEFAULT_CATEGORIES, EventStreamPrinter
from threatlens.ui.formatting import format_bytes, sanitize_display
from threatlens.ui.json_output import connection_record, envelope, json_line, write_json
from threatlens.ui.network_views import (
    connection_chain_text,
    connections_table,
    process_connections_tree,
    process_network_table,
)
from threatlens.ui.process_views import (
    describe_unverified_parent,
    process_detail,
    process_table,
    process_tree,
)
from threatlens.ui.response_views import action_result_text, action_warning
from threatlens.ui.status_view import status_view
from threatlens.ui.storage_views import (
    db_info_view,
    event_row_json,
    events_table,
    persistence_table,
)
from threatlens.utils.time import parse_duration, utc_now
from threatlens.utils.windows import get_windows_version

logger = logging.getLogger("threatlens.cli")
    ...
    def open_store(self, *, read_only: bool = False) -> SecurityStore:
        ...
        if read_only and not path.exists():
            raise ThreatLensError(
                f"No database at {path}. Run 'threatlens monitor' to record activity first."
            )
...
def _load_allowlist(ctx: AppContext) -> AllowlistMatcher | None:
    ...
    try:
        ...
        try:
            ...
            store.db.close()
    except (OSError, ThreatLensError):
        return None
...
def cmd_monitor(ctx: AppContext) -> int:
    ...
    with InstanceLock(app_data_dir() / LOCK_FILE_NAME):
        ...
        if printer is not None and human:
            ctx.err_console.print(
                Text(
                    f"ThreatLens monitor · every {engine.interval:g}s · "
...
def cmd_rules(ctx: AppContext) -> int:
    ...
    if rule_id is not None:
        ...
        if meta is None:
            raise InvalidInputError(
                f"Unknown rule {rule_id!r}. Run 'threatlens rules' to list available rules."
            )
    ...
    if not ctx.quiet:
        ctx.console.print(
            Text(
                ...
                "verdicts · 'threatlens rules show <RULE_ID>' for details",
                style=colors.MUTED,
            )
        )
    return ExitCode.OK
...
def cmd_firewall(ctx: AppContext) -> int:
    ...
    if action == "list":
        try:
            ...
            raise ThreatLensError(str(exc)) from exc
        if ctx.json:
            write_json(envelope("firewall_rules", rules=rules), sys.stdout)
        elif rules:
            for name in rules:
                ctx.console.print(Text(name))
            ctx.console.print(Text(f"{len(rules)} ThreatLens firewall rules", style=colors.MUTED))
        else:
            ctx.console.print(Text("No ThreatLens firewall rules.", style=colors.MUTED))
        return ExitCode.OK
    ...
    if result.outcome is ActionOutcome.SUCCEEDED and not ctx.quiet and not ctx.json:
        ctx.console.print(
            Text(
                f'Reverse with: threatlens firewall unblock "{spec.rule_name}"', style=colors.MUTED
            )
        )
    return _ACTION_EXIT[result.outcome]
...
def _maybe_record_firewall(
    ...
    *,
    ...
    if action.outcome is not ActionOutcome.SUCCEEDED:
        return
    with contextlib.suppress(OSError, ThreatLensError):
        store = ctx.open_store()
        try:
...
def cmd_alerts(ctx: AppContext) -> int:
    ...
    try:
        if action == "set-status":
            ...
            if not ok:
                raise ThreatLensError(f"No alert matching {ctx.args.alert_id!r}")
            if not ctx.quiet:
                ctx.console.print(
                    Text(
    ...
    if not alerts:
        ctx.console.print(
            Text("No alerts match. Run 'threatlens monitor' to generate them.", style=colors.MUTED)
        )
        return ExitCode.OK
    ...
    if not ctx.quiet:
        ctx.console.print(
            Text(
                f"{len(alerts)} alerts · 'threatlens alerts show <ID>' for detail · "
                "'threatlens alerts set-status <ID> <STATUS>' to triage",
                style=colors.MUTED,
            )
        )
    return ExitCode.OK
    ...
    try:
        if action == "list":
            ...
            if ctx.json:
                ...
                ctx.console.print(
                    Text(
                        "No baselines. Create one with 'threatlens baseline create'.",
                        style=colors.MUTED,
                    )
                )
            return ExitCode.OK
        if action == "delete":
            ...
            if not ok:
                raise ThreatLensError(f"No baseline {ctx.args.baseline_id!r}")
            if not ctx.quiet:
                ctx.console.print(
        ...
        if row is None:
            raise ThreatLensError(
                "No baseline to compare against. Run 'threatlens baseline create' first."
            )
        baseline_items = [
            BaselineItem(
...
def cmd_allow(ctx: AppContext) -> int:
    ...
    if action == "remove":
        ...
        try:
            ...
            store.db.close()
        if not ok:
            raise ThreatLensError(f"No allowlist entry {ctx.args.match_type} {ctx.args.value!r}")
        if not ctx.quiet:
...
def cmd_logs(ctx: AppContext) -> int:
    ...
    try:
        ...
        raise ThreatLensError(f"Cannot read log file {path}: {exc}") from exc
...
def cmd_db(ctx: AppContext) -> int:
    ...
    if _engine_running(ctx):
        raise ThreatLensError("A monitor is running; stop it before 'db cleanup' or 'db vacuum'.")
    store = ctx.open_store()
    try:
        if action == "cleanup":
            deleted = store.apply_retention(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="threatlens",
        description=(
            "ThreatLens — see what your Windows system is doing, detect what shouldn't be "
            "happening, respond safely."
        ),
        epilog="Run 'threatlens <command> --help' for command details.",
    )
    ...
    p = sub.add_parser(
        ...
        description=(
            "Block outbound traffic by IP, port or program via the Windows Firewall. ThreatLens "
            "only adds rules named 'ThreatLens:…' and only removes those, so it can never weaken "
    ...
    fw_list = fw_sub.add_parser("list", parents=[common], help="list ThreatLens firewall rules")  # noqa: F841
    ...
    fw_unblock = fw_sub.add_parser(
        "unblock", parents=[common], help="remove a ThreatLens firewall rule"
    )
...
def _dispatch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return ExitCode.OK  # e.g. `threatlens processes | Select-Object -First 5`
