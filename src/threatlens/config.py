...
* If ThreatLens runs elevated but reads a config file writable by a standard user, a
...
CONFIG_ENV_VAR: Final = "THREATLENS_CONFIG"
LOG_LEVEL_ENV_VAR: Final = "THREATLENS_LOG_LEVEL"
MAX_CONFIG_BYTES: Final = 256 * 1024
APP_DIR_NAME: Final = "ThreatLens"


def app_data_dir() -> Path:
    """Per-user data directory: ``%LOCALAPPDATA%\\ThreatLens``.
...
class GeneralConfig(_Section):
    ...
    def resolved_database_path(self) -> Path:
        return Path(self.database_path) if self.database_path else app_data_dir() / "threatlens.db"

    def resolved_log_path(self) -> Path:
        return Path(self.log_path) if self.log_path else app_data_dir() / "threatlens.log"
...
class EngineConfig(_Section):
    """Runtime limits for ``threatlens monitor``. Every queue and cache is bounded."""
...
# ThreatLens configuration. All keys are optional; values shown are the defaults.

[general]
refresh_interval_seconds = 2.0
log_level = "INFO"                 # DEBUG | INFO | WARNING | ERROR
# database_path = "%LOCALAPPDATA%\\\\ThreatLens\\\\threatlens.db"
# log_path = "%LOCALAPPDATA%\\\\ThreatLens\\\\threatlens.log"
...
disabled_rules = []                # e.g. ["NET-004"]; see 'threatlens rules'
