...
def _describe(exc: BaseException) -> str:
    ...
    return (
        message
        if type(exc).__module__.startswith("threatlens")
    ...
    def start(self, *, run_immediately: bool = True) -> None:
        ...
        for runner in self._runners:
            thread = threading.Thread(
                ...
                name=f"threatlens-{runner.name}",
                daemon=True,
            )
            self._threads[runner.name] = thread
            thread.start()
        watchdog = threading.Thread(target=self._watchdog, name="threatlens-watchdog", daemon=True)
