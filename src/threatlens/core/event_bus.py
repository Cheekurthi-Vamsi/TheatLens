...
* **Bounded.** The queue holds at most ``capacity`` events. When full, the *oldest* event is
  ...
  memory stays bounded, and the drop counter surfaces in ``threatlens status`` so loss is never
  silent.
...
from threatlens.core.models import BusStats, EventType, SecurityEvent
...
class EventBus:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="threatlens-dispatcher", daemon=True)
        self._thread.start()
