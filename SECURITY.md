...
ThreatLens is a **defensive**, local-first host monitor intended for your own Windows machines or
systems you are authorised to monitor.

## What ThreatLens will never do

ThreatLens does not hide itself, evade or disable security products (including Windows Defender
...
## Security design of ThreatLens itself
...
| Crashing Windows by killing a critical process | Protected-process policy refuses suspend/terminate for kernel pseudo-processes, `csrss`/`wininit`/`winlogon`/`services`/`lsass`/`smss`, SYSTEM-integrity processes, configured names, and ThreatLens itself; override needs `--force-protected` **and** confirmation |
...
| Weakening the firewall | ThreatLens only adds block rules named `ThreatLens:…` and refuses to delete any rule without that prefix; each change is audited and reversible with `firewall unblock` |
...
| Detection evasion by naming a binary like ThreatLens | Self-exclusion is by exact process instance (PID + creation time) of the running engine and its launcher chain only — never by name or path |
...
* **`detection.ignored_executables` and `detection.trusted_paths` reduce visibility.** Anyone who can
  ...
  the account that runs ThreatLens.
...
* **Elevated use with a user-writable config.** If ThreatLens runs elevated but reads
  `%LOCALAPPDATA%\ThreatLens\config.toml`, any process running as that user could weaken settings.
...
public issue. Include the ThreatLens version (`threatlens --version`), Windows build, and
